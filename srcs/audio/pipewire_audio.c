...
/*   Updated: 2025/06/20 00:28:01 by amagno-r         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void on_process(void *userdata)
{
	...
	if ((b = pw_stream_dequeue_buffer(pw_data->stream)) == NULL) {
		printf("PipeWire: out of buffers\n");
		return;
	}
...
static void on_param_changed(void *userdata, uint32_t id, const struct spa_pod *param)
{
	...
	printf("PipeWire: Format changed - rate: %d, channels: %d\n", 
...
void *pipewire_audio_thread(void *arg)
{
	...
	if (!pw_data.loop) {
		printf("Failed to create PipeWire main loop\n");
	...
	if (pw_stream_connect(pw_data.stream,
			...
			params, 1) < 0) {
		printf("Failed to connect PipeWire stream\n");
